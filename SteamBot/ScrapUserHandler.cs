...
namespace SteamBot
{
    public class ScrapUserHandler : UserHandler
    {
        ...
        ScrapUserHandlerConfiguration ConfigObject;
        ...
        public ScrapUserHandler(Bot bot, SteamID sid)
            : base(bot, sid)
        {
            if(string.IsNullOrWhiteSpace(bot.UserHandlerConfigName))
            {
                Console.WriteLine("Please specify a UserHandlerConfigName for this bot in your settings.json.", bot.UserHandlerConfigName);
                return;
            }
            if (!File.Exists(bot.UserHandlerConfigName))
            {
                Console.WriteLine("No {0} file found.", bot.UserHandlerConfigName);
                ScrapUserHandlerConfiguration.WriteConfiguration(new ScrapUserHandlerConfiguration(), bot.UserHandlerConfigName);
                return;
            }

            try
            {
                ConfigObject = ScrapUserHandlerConfiguration.LoadConfiguration(bot.UserHandlerConfigName);
            }
            catch (Newtonsoft.Json.JsonReaderException)
            {
                // handle basic json formatting screwups
                Console.WriteLine("{0} file is corrupt or improperly formatted.", bot.UserHandlerConfigName);
                return;
            }
        }
        ...
        public override void OnMessage(string message, EChatEntryType type)
        {
            ...
            else if (message == ".version")
            {
                if (IsAdmin)
                {
                    Bot.SteamFriends.SendChatMessage(currentSID, EChatEntryType.ChatMsg, "Version: " + BotVersion);
                }
            }
            else
            {
        ...
        public override void OnTradeInit()
        {
            ...
            Trade.SendMessage(ConfigObject.TradeGreetMessage);
            TradeCountInventory(true);
            Trade.SendMessage(ConfigObject.TradeGreetMessage2);
        }
